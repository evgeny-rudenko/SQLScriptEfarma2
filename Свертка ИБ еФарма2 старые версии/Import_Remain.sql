...
/*
модификация стандартной процедуры 
теперь  все работает без мнемокодов 
...
FROM _IMPORT_REMAINS R
   INNER JOIN GOODS G ON CAST(R.OUTER_CODE AS VARCHAR(20)) = G.MNEMOCODE
...
SELECT
    ...
    N.QUANTITY,
    N.DELEN,
    N.SERIES ,--LTRIM(RTRIM(N.SERIES)) AS SERIES,
    N.BESTBEFORE,
    N.SUP_PRICE,
    N.RET_PRICE,
    N.TAXRATER,
...
FROM _IMPORT_REMAINS N
    ...
    --LEFT JOIN GOODS G ON G.MNEMOCODE = CAST(N.OUTER_CODE AS VARCHAR(20))
    ...
    WHERE ID_CONTRACTOR = @ID_CONTRACTOR AND ID_STORE = @ID_STORE
        ...
        -- ВСТАВКА СТРОК НАКЛАДНОЙ
            ...
            SELECT
                ...
                QUANTITY * RET_PRICE,
                BAR_CODE, NULL, ID_SERIES_LOC, ID_GOODS, '',
                TAXRATES,
                TAXRATER,
                ID_INVOICE_ITEM_GLOBAL,
				TAXRATES				
